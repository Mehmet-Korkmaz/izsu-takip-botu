...
@app.route('/')
def home():
    return "İZSU Takip Botu Aktif ve Akıllı Komut Sistemi Çalışıyor!"
...
def db_kur():
    """Hafıza için küçük ve kalıcı bir veri tabanı tablosu oluşturur."""
    conn = sqlite3.connect(DB_DOSYASI)
    cursor = conn.cursor()
    cursor.execute('''CREATE TABLE IF NOT EXISTS konum 
                      (id INTEGER PRIMARY KEY, ilce TEXT, mahalle TEXT)''')
    # Eğer ilk defa kuruluyorsa varsayılan değerleri ekle
...
def telegram_komut_dinle():
    """Telegram'dan gelen /konum mesajlarını sürekli dinler."""
    son_update_id = 0
...
# Botu ve İZSU kontrolünü arka planda ayrı ayrı başlatıyoruz
